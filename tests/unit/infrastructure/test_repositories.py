...
2 47563  53.0540  100.1234   0001500  90.0000 270.1234  15.06390000123456"""
    ...
    @patch("requests.get")
    def test_get_active_satellites_success(self, mock_get, mock_tle_data):
        """測試成功獲取活躍衛星"""
        # 設置模擬響應
        mock_response = Mock()
        ...
        satellites = repo.get_active_satellites()
    ...
    @patch("requests.get")
    def test_get_active_satellites_api_error(self, mock_get):
        """測試 API 錯誤處理"""
        # 模擬 API 錯誤
    ...
    @patch("requests.get")
    def test_get_active_satellites_invalid_response(self, mock_get):
        """測試無效響應處理"""
        # 模擬無效的 TLE 數據
        mock_response = Mock()
        mock_response.status_code = 200
    ...
    @patch("requests.get")
    def test_get_satellite_by_id_found(self, mock_get, mock_tle_data):
        """測試通過 ID 獲取衛星 - 找到"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.text = mock_tle_data
        ...
        repo = CelestrakSatelliteRepository()
        satellite = repo.get_satellite_by_id("47562")
    ...
    @patch("requests.get")
    def test_get_satellite_by_id_not_found(self, mock_get, mock_tle_data):
        """測試通過 ID 獲取衛星 - 未找到"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.text = mock_tle_data
        ...
        repo = CelestrakSatelliteRepository()
        satellite = repo.get_satellite_by_id("99999")
    ...
    def test_parse_tle_valid(self):
        ...
        satellite = repo._parse_tle(tle_lines)
    ...
    def test_parse_tle_invalid(self):
        ...
        # 無效的 TLE 格式
        invalid_tle_lines = ["INVALID", "NOT A TLE", "INVALID DATA"]

        satellite = repo._parse_tle(invalid_tle_lines)
        assert satellite is None

    @patch("requests.get")
    def test_get_satellites_by_name_pattern(self, mock_get, mock_tle_data):
        """測試通過名稱模式獲取衛星"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.text = mock_tle_data
        ...
        repo = CelestrakSatelliteRepository()
        satellites = repo.get_satellites_by_name_pattern("STARLINK")

        assert len(satellites) == 2
        assert all("STARLINK" in sat.name for sat in satellites)

    def test_tle_cache(self):
        """測試 TLE 緩存機制"""
...
2 47562  53.0540  100.1234   0001500  90.0000 270.1234  15.06390000123456"""
            ...
            # 第一次調用
            satellites1 = repo.get_active_satellites()
            assert len(satellites1) == 1

            # 第二次調用應該使用緩存
            satellites2 = repo.get_active_satellites()
