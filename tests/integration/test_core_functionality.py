...
def test_prediction_service():
    ...
    try:
        prediction = prediction_service.predict_coverage(
            satellites=satellites, observer=observer, time_scale=PredictionTimeScale.SHORT_TERM
        )
    ...
    try:
        ...
        uncertainty_1h = prediction_service.calculate_prediction_uncertainty(
            prediction_time=datetime.now(timezone.utc).replace(hour=(datetime.now().hour + 1) % 24), base_time=base_time
        )
        print(f"   ✓ 1小時後不確定性: {uncertainty_1h}")

        # 24小時後的不確定性
        uncertainty_24h = prediction_service.calculate_prediction_uncertainty(
            prediction_time=datetime.now(timezone.utc).replace(day=datetime.now().day + 1), base_time=base_time
        )
