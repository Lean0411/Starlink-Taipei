...
class OrbitalElements:
    ...
    def __post_init__(self):
        ...
        if not 0 <= self.inclination <= 180:
            raise ValueError(f"軌道傾角必須在 0 到 180 度之間")
        
        if not 0 <= self.raan <= 360:
            raise ValueError(f"升交點赤經必須在 0 到 360 度之間")
        
        if not 0 <= self.eccentricity < 1:
            raise ValueError(f"離心率必須在 0 到 1 之間（不含 1）")
        
        if not 0 <= self.arg_perigee <= 360:
            raise ValueError(f"近地點幅角必須在 0 到 360 度之間")
        
        if not 0 <= self.mean_anomaly <= 360:
            raise ValueError(f"平近點角必須在 0 到 360 度之間")
        
        if self.mean_motion <= 0:
            raise ValueError(f"平均運動必須大於 0")
