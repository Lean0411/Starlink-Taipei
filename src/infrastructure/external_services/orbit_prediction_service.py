...
class OrbitPredictionService(PredictionService):
    ...
    def predict_coverage(
        ...
        while current_time <= end_time:
            prediction_point = self._predict_at_time(
                satellites, observer, current_time, start_time
            )
    ...
    def _predict_at_time(
        ...
        for satellite in satellites:
            try:
                # 計算衛星在預測時間的位置
                position = self.orbit_calculator.calculate_position(
