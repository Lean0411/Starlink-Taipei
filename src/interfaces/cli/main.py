...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Starlink 衛星覆蓋率分析工具", formatter_class=argparse.RawDescriptionHelpFormatter
    )

    # 位置參數
    parser.add_argument("--lat", type=float, default=25.0330, help="觀測者緯度（預設：台北）")
    parser.add_argument("--lon", type=float, default=121.5654, help="觀測者經度（預設：台北）")
    parser.add_argument("--elevation", type=float, default=10.0, help="觀測者高度，公尺（預設：10）")

    # 時間參數
    parser.add_argument("--duration", type=int, default=60, help="分析持續時間，分鐘（預設：60）")
    parser.add_argument("--interval", type=int, default=1, help="時間間隔，分鐘（預設：1）")
    parser.add_argument("--start-time", type=str, help="開始時間（ISO格式），預設為現在")

    # 篩選參數
    parser.add_argument("--min-elevation", type=float, default=25.0, help="最小仰角，度（預設：25）")
    parser.add_argument("--satellite-filter", type=str, help="衛星名稱篩選（支援通配符）")

    # 輸出參數
    parser.add_argument("--output", type=str, help="輸出檔案路徑")
    parser.add_argument("--format", choices=["json", "summary"], default="summary", help="輸出格式（預設：summary）")
