...
from app.utils import (
    ...
    handle_errors, ErrorContext, ConfigurationError
)
...
使用範例:
  ...
  %(prog)s health                    # 健康檢查
    ...
    shiny_parser.add_argument('--host', type=str, default='0.0.0.0',
                             help='網頁應用主機（預設0.0.0.0）')
    shiny_parser.add_argument('--simple', action='store_true',
                             help='啟動簡化版界面')
...
def run_analyze(args):
    """執行衛星分析"""
    with ErrorContext("satellite_analysis", 
        ...
        if not Path('satellite_analysis_updated.py').exists():
            if not Path('satellite_analysis.py').exists():
                raise ConfigurationError(
                    "找不到衛星分析模組",
                    details={'files_checked': ['satellite_analysis_updated.py', 'satellite_analysis.py']}
                )
        ...
        cmd = [
            ...
            '--min-elevation', str(args.min_elevation)
        ]
...
def run_shiny(args):
    ...
    with ErrorContext("shiny_app", port=args.port, simple=args.simple):
        ...
        if not Path(app_file).exists():
            raise ConfigurationError(
                f"找不到 Shiny 應用檔案: {app_file}",
                details={'file': app_file}
            )
