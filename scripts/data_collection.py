...
class HistoricalDataCollector:
    ...
    def collect_daily_data(
        ...
        try:
            ...
            for hour in range(24):
                ...
                try:
                    # 分析該時間點的衛星狀態
                    visible_sats = self.analyzer.get_visible_satellites_at_time(
                        analysis_time
                    )
    ...
    def collect_historical_range(
        ...
        """收集指定時間範圍的歷史數據"""
        logger.info(
            f"開始收集 {start_date.strftime('%Y-%m-%d')} 到 {end_date.strftime('%Y-%m-%d')} 的數據"
        )
        ...
        while current_date <= end_date:
            ...
            output_file = self.collect_daily_data(
                current_date, observer_lat, observer_lon
            )
            if output_file:
    ...
    def generate_dataset_summary(self, data_files: list):
        ...
        for file_path in data_files:
            try:
                ...
                file_info = {
                    ...
                    "satellites_count": sum(
                        len(obs.get("satellites", [])) for obs in data
                    ),
                }
        ...
        print(
            f"日期範圍: {summary['statistics']['date_range']['start']} 到 {summary['statistics']['date_range']['end']}"
        )
